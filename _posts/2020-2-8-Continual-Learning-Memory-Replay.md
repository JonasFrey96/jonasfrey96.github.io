...
Where $$ l $$ is the loss function. Finding the best current weights $$\theta^t$$ is constrained to not increase the loss on previously seen samples $$ i \in [0..t-1] $$.
...
The first order gradient with respect to the model's parameters is calculated over the loss induced by the sum of all elements in the batch. When we now differentiate between replayed and new samples we can assign explicitly the contribution to the weight updated split into new and replayed samples.

$$ \theta_{t+1} = w_t + \mu( \frac{\theta}{d\theta} \Sigma_i^{new} l(f_w(x_i),y_i) + \frac{\theta}{d\theta} \Sigma_j^{old} l(f_\theta(x_j),y_j) )$$ 

We enumerate samples from a previous task using j and i for the new tasks.
...
## Experiments
By just looking at a few equations and properties of the optimization procedure, used to train neural networks we were facing multiple questions on how we actually implement memory replay and what kind of hyper-parameters we should be aware of.

## Experiment Setup:
To conduct experiments we have chosen the semantic segmentation network  [Fast-SCNN](https://arxiv.org/abs/1902.04502). This network is relatively small and shows good performance while being extremely fast during inference. Also the architecture is separated into different modules which produce feature representations at different scales, which we will use later. 
...
The images are rendered photorealistically and are separated into different scenes. A total of 461 indoor scenes are available with multiple camera trajectories with dense ground truth annotations for depth and semantic labels. The objects in the scene are labeled according to the NYU40 classes.
...
Each task consists of ~2000 training images and ~500 validation images.
...
When continual learning is implemented successfully this allows to improve the semantic segmentation estimate while on the mission when revisiting the same street in the city while preserving on knowledge (4).
...
We can observe that some tasks are harder than other ("Lower accuracy") but all are high.
[ 0.84, 0.84, 0.73, 0.78]
...
We use an effective batch size of 64 and use a data distributed parallel training backend. For more implementation details we refer to the publicly available code on [https://github.com/JonasFrey96/ASL](https://github.com/JonasFrey96/ASL).
...
We now implement a **template random replay buffer**. For each task we create a buffer which we randomly populate with samples from the current task. When we train on the next task we randomly sample with a probability *p* an element from the buffers of the previous tasks.
...
When training a new task we reset the counter for both new and reharseled samples. We can observe that the slope of the new samples is decreasing while the slope for rehearsal elements is increasing. 

In the matrix-result block we can observe that all lower diagonal elements increased significantly. Despite this we are not able to match optimal solution which was achieved by training simultaneously on all tasks.
...
In the prior experiments we have seen a naive and optimal baseline. We have also observed that a random memory buffer is able to avoid catastrophic forgetting. In the following we will perform modifications to the template random replay buffer. This allows us to discuss the importance of certain parameters and design choices.
...
As we have seen previously there are multiple design considerations when we think about sampling from the buffer. Previously we have seen adaptive sampling where the probability of obtaining a buffer sample is chosen individual for each task according to the following formula:
...
We also perform experiments with different fixed rehearsal probabilities with probability p_{fix}. 

The downside of this method is that after training the 4 tasks a sample from task 1 is visited more often then a sample from the final task.
...
#### Step based
One very important factor when training for a new task is the number of steps/epochs. When observing the learning progress of the validation accuracy we can clearly correlate performance increase on the current task with performance decrease on previous task. To showcase the significance of the results we have trained the network with the template replay buffer method for different amounts of epochs per task. We have chosen, [10,50,100]. Here one **step based epoch** is defined as fixed number of training steps =  |Task_i|/BS.

To avoid the dependency on the rehearsal probability we can define an **effective epoch**. This is defined by the number of times a sample from the current task has been fed to the network.
For example with a rehearsal probability of 0.5 when training for task 1 with 2000 samples and being in **Step based epoch**=10. The network was trained with 2000/BS x 10 update steps. Given the rehearsal probability of 0.5 each sample was seen 10x(1-0.5)=5 times by the network. This leads to 5 effective epochs.

#### Metric based
<!-- When looking at the results in matrix from we can see that the diagonal elements (performance on the currently active task) increases with training time and the knowledge preservation decreases (lower diagonal elements).  -->
...
Instead of using the validation accuracy on the current task we can limit the amount of "acceptable forgetting".
...
### Augmentation
A key element we observed while training the neural network is data augmentation. Data augmentation is shown to avoid overfitting and increase the generalization capability of neural networks. This is especially useful in scenarios where limited data is available.
The transformations such as color jitter, illumination changes or random cropping for vision tasks should not change the network prediction. Therefore we motivate the network to become invariant to these transforms. We can encode our prior domain knowledge in the neural network by designing augmentation methods that preserve the label. 
