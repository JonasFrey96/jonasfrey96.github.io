...
<img src="{{ site.baseurl }}/images/traget_train_domain.jpg" alt="drawing" height="300"/>
...
You were able to caputure datapoints (red) withing your training set such that they cover the same space as the target data distribution. We can assume that a network that performs well on the training data generalizes to the target data.

At the ofter right a domain shift between the target and training domain is present.
We dont expect a model trained on the training data to generalize to the test data.
This is for example the case when you train on data generated in a simulator and perform inference on real world data. (Sim-To-Real Usecase) 
...
Given that for certain applications it is not possible to create a training datasets that covers all information that is needed in the future a dire need for a tool or procedure that allows to continually integrate new information into a network is present. **Continual learning faces this challenge**
...
You are right! This procedure is currently executed when a network is employed in real-world applications. The networks are either fine-tuned or fully retrained on an enriched dataset that includes the new data. A hole ecosystem and tool-chains have been created to manage datasets and constant retraining and employing of updated networks.
...
In the above figure the strength of the lines visualize the weight between two fully-connected layer. When the network is trained the weights of these connection changes such that new information is acquired
...
### **Task**
We define a task T_i as a learning objective, where data x should be mapped to the label y. This learning objective correspondences to finding a target function g: g(x)=y.
...
### **Supervison**  
<img src="{{ site.baseurl }}/images/supervision.jpg" alt="drawing"/>  
...
One example might be when performing object classification in images. Here we know it dose not matter where a object is present in the image. 
...
This is not tha case for stream learning where for example data is acquired by a continual sensory input such as a camera video stream. 
...
The idee of rehearsal methods is to gather samples from previous tasks. These samples can then be mixed with the new data. Therefore during training the weight update includes samples from previous tasks, which will motivate the network to perform well on the stored sampled as well as the new data. Storing the samples from previous tasks is memory intensive. For most applications carefully selected subset of the data of the previous task is chosen to be stored. 
With growing number of tasks the training procedure takes longer given that more data from previous tasks has to be replayed.
...
In contrast to normal Rehearsal approach the samples of a old task can be generated by sampling from the generator network. 
...
Training a generative network form the perspective of information theory can be interpreted as compressing the previous task data into the model weights of the generator.
